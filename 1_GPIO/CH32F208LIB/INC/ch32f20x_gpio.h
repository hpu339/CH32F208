...
void GPIO_DeInit(GPIO_TypeDef* GPIOx);                       //将GPIO恢复到默认状态（失能）
void GPIO_AFIODeInit(void);     //取消初始化复用功能
void GPIO_Init(GPIO_TypeDef* GPIOx, GPIO_InitTypeDef* GPIO_InitStruct);//GPIO初始化
void GPIO_StructInit(GPIO_InitTypeDef* GPIO_InitStruct);//结构体默认初始化
uint8_t GPIO_ReadInputDataBit(GPIO_TypeDef* GPIOx, uint16_t GPIO_Pin);//输入模式：读取引脚高低电平状态
uint16_t GPIO_ReadInputData(GPIO_TypeDef* GPIOx);//输入模式：读取整组GPIO状态
